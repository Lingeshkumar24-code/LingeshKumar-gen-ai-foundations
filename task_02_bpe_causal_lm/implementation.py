    ...
    def train(self, texts, num_merges=50):
        ...
        for _ in range(num_merges):
            ...
            for wi,w in enumerate(words):
                ...
                words[wi]=out
            if merged not in self.vocab: self.vocab[merged]=len(self.vocab)
        return self
...
class CausalLM(nn.Module):
    def __init__(self,vocab_size,d=128):
        super().__init__(); self.emb=nn.Embedding(vocab_size,d); self.proj=nn.Linear(d,vocab_size)
    def forward(self,x):
        n=x.size(1); mask=torch.triu(torch.full((n,n),float('-inf')),1).to(x.device)
        h=self.emb(x); a=torch.softmax(h@h.transpose(-1,-2)/d**0.5+mask,dim=-1); return self.proj(a@h)

if __name__=='__main__':
    texts=['transformer self attention','causal language model','generative artificial intelligence']
    b=BPE().train(texts,20); data=torch.tensor([b.encode(t) for t in texts],dtype=torch.long)
    d=128; model=CausalLM(len(b.vocab),d); opt=torch.optim.AdamW(model.parameters(),lr=3e-3); loss_fn=nn.CrossEntropyLoss()
    for _ in range(3):
